    ...
    if (step === 1) {
      if (
        ...
        try {
          ...
          await axios
            .post(
              `${process.env.NEXT_PUBLIC_IBENDOUMA_CLIENT_URL}/users/send-user-verification-code`,
              {
      ...
      if (formData.code.length < 6) {
        ...
        try {
          ...
          await axios
            .get(
              `${process.env.NEXT_PUBLIC_IBENDOUMA_CLIENT_URL}/code/verify/${formData.code}`
            )
            .then((response) => {
              if (response.data) {
                setLoading(false);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      try {
        ...
        await axios
          .post(
            `${process.env.NEXT_PUBLIC_IBENDOUMA_CLIENT_URL}/users/register`,
            data
          )
          .then((response) => {
            if (response.data) {
